...
std::vector<Vec3> Plane::generateWindVelocities(Vec3 windDir, int clock)
{
    ...
    for(i = 0; i < fieldWidth; i++)
    {
        ...
        {
            dist = abs( windDir.z * (float)i * m_scale + windDir.x * (float)j * m_scale ) / sqrt(windDir.z*windDir.z + windDir.x*windDir.x);
            windStrength = sin( (dist/maxWidth + (float)clock/25.0f) * m_windSpread )*0.5f+0.5f;
